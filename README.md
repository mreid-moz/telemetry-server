...
1. Telemetry pings are generated by Firefox instances and are sent out from users' computers.
    * To simulate this for testing purposes, run `python test/submit_pings.py FOLDER_CONTAINING_PINGS` to submit all the pings in the specified folder.
    ...
    * The `test/sample-saved_session-pings` folder can be used as an example: run `python test/submit_pings.py test/test-saved_session-pings` while the Node server is running.
2. The pings are received by a Node server (which can be found in `http/server.js`).
    ...
    * An example stored ping can be found at `test/work/telemetry.log.v2.osboxes.1965.1432830357331.finished`.
3. The Node server writes the pings to storage, batching them together in files up to the configured limit. Each file can contain multiple pings.
    * Test pings should be submitted using the `test/submit_pings.py` script - execute it with the name of a folder containing JSON pings (each ping's filename must end in `.json`).
    * When dealing with pings submitted using the `test/submit_pings.py` script, each resulting file is an LZMA compressed text file, with one ping per line.
4. Via a queue, the incoming ping processor reads these compressed pings, converts them into a more compact format, then outputs them as LZMA compressed compacted JSON pings.
    * To simulate this for testing purposes, simply run `test/convert_pings_to_compacted_pings.sh` to process all the pings (in the format generated by the Node server) in `test/work`, writing the results to `test/out`.
    * This runs `process_incoming/process_incoming_standalone.py` with the `--dry-run` argument, which avoids the use of SQS.
    * To view the results in plaintext, run `test/unpack_compacted_pings.sh` in the same folder as the `out` directory. The human-readable outputs can be found in `test/test-ping-outputs`.
