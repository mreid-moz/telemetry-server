...
def build_config(job):
    if job["code_uri"].endswith(".ipynb"):
        ...
        config = {
            ...
            "image": "ami-5d4a7b6d", # -> telemetry-worker-hvm-20150504 (Ubuntu 14.10)
...
def monitor(instance_id):
    ...
    terminate_time = parse_date(instance.launch_time, ignoretz = True) + timedelta(days = 1)
...
def cluster_monitor(jobflow_id):
    ...
    # Instance gets killed by terminate-expired-instances.py 1 day after the creation time
    creationdatetime = getattr(jobflow, "creationdatetime", datetime.utcnow())
    terminate_time = parse_date(creationdatetime, ignoretz = True) + timedelta(days = 1)
